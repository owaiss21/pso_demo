...
def draw_track(frame, xyxy, logical_id: int, track_id: int, role: str):
    ...
    # role-aware colors
    color = (0, 255, 0) if role.lower() == "customer" else (255, 128, 0)  # green / orange
...
class LogicalIDManager:
    ...
    def assign(self, frame_bgr, detections, t_now, customer_tri, exit_tri,
        ...
        for det in detections:
            ...
            if obj.last_box is None:
                obj.last_box = np.array(box, dtype=float)
            else:
    ...
    def ready_to_log(self, lid, t_now):
        ...
        if not obj or obj.logged or obj.customer_entry_start_t is None:
            return False
        # Log for BOTH Customer and Employee now
...
def main(
    ...
    min_aspect: float = 1.2,                 # h/w lower bound (person-like boxes are tall)
    ...
    events_csv_path = str(run_root / "human_events.csv")  # renamed file for clarity

    # ==== Load YOUR YOLOv11 weights here ====
    ...
    try:
        while True:
            ...
            if need_infer:
                # Track ALL classes; we will route humans vs others in code
                results = model.track(
                    frame,
                    persist=True,
                ...
                if results and hasattr(results[0], "boxes") and results[0].boxes is not None:
                    ...
                    if hasattr(boxes, "cls") and boxes.cls is not None:
                        clses = boxes.cls.cpu().numpy().astype(int).flatten() if hasattr(boxes.cls, "cpu") else boxes.cls.astype(int).flatten()
                    ...
                    for i in range(n):
                        ...
                        if cls_id in HUMAN_CLASS_IDS:
                            ...
                            human_detections.append({"track_id": track_id, "box": box_xyxy, "role": role, "cls_id": cls_id})
                        else:
                            other_detections.append({"box": box_xyxy, "label": label})

                # Logical-ID assignment ONLY for human classes
                assigned = lidm.assign(
                    frame, human_detections, t_now, customer_tri, exit_tri,
                ...
                for det in assigned:
                    ...
                    if lidm.ready_to_log(lid, t_now):
                        ...
                        event_name = f"{det['role'].lower()}_at_counter"
                        writer.writerow([
                            event_name, lid, tid, video_time_str, system_time_str,
                            orig_idx, det["role"], x1, y1, x2, y2
                        ])
                        f_csv.flush()
                        lidm.mark_logged(lid)

                # Draw all other classes (simple box + label)
                for od in other_detections:
                    draw_box_with_label(frame, od["box"], od["label"])

                last_infer_t = t_now
                if not infer_every_frame:
                    next_infer_t += infer_dt

            # write output video & frame exports
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="BoT-SORT (strict) + motion-aware logical IDs + quality gating; entry ROI excludes top-right exit wedge. Uses YOLOv11 weights and tracks Customer/Employee."
    )
    ...
    parser.add_argument("--min-aspect", type=float, default=1.2, help="Min h/w for person-like boxes")
    parser.add_argument("--max-aspect", type=float, default=5.0, help="Max h/w for person-like boxes")
